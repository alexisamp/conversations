...
function groupWindows(messages: BridgeMessageRow[]): Window[] {
  ...
  for (const [chatId, rows] of byChat) {
    ...
    for (const row of sorted) {
      if (current.length > 0 && row.timestamp_ms - start > WINDOW_MS) {
        windows.push({
          chatId,
...
export class DailyInsightRunner {
  ...
  private async runMessages(messages: BridgeMessageRow[], reason: string): Promise<InsightRunResult> {
    ...
    try {
      ...
      for (const win of windows) {
        ...
        await supabase.from('outreach_logs')
          .update({
            last_interaction_at: new Date(first.timestamp_ms).toISOString(),
        ...
        if (interaction?.id) {
          await supabase.from('extension_interaction_windows').insert({
            ...
            window_end: new Date(last.timestamp_ms + WINDOW_MS).toISOString(),
            direction: dominantDirection(win.messages),
            message_count: win.messages.length,
          }).then(({ error }) => {
            if (error) console.warn('[daily-insights] window insert failed:', error.message)
          })
        }
        ...
        for (const fact of extraction.contact_facts.slice(0, 5)) {
          ...
          if (fact.needs_review) {
            await this.createReviewItem(user.id, key + ':fact:' + fact.value.slice(0, 40), {
        ...
        recordAiOutput(key, 'interaction', interaction?.id ?? null)
        markBridgeMessagesSynced(win.messages.map((message) => message.id), contactId)
      ...
      return { runId, messagesSeen: messages.length, conversationsProcessed, outputsWritten }
    } catch (err) {
